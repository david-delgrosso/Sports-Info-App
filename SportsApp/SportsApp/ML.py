...
class NBAElnReg:
    def __init__(self):
        self.home_filename   = "/home/davidm97/Projects/Sports-Info-App/SportsApp/SportsApp/ml_models/nba_eln_reg_home_points.sav"
        self.away_filename   = "/home/davidm97/Projects/Sports-Info-App/SportsApp/SportsApp/ml_models/nba_eln_reg_away_points.sav"
        self.scaler_filename = "/home/davidm97/Projects/Sports-Info-App/SportsApp/SportsApp/ml_models/nba_scaler.sav"

        self.home_eln_reg  = pickle.load(open(self.home_filename, 'rb'))
        self.away_eln_reg  = pickle.load(open(self.away_filename, 'rb'))
        self.scaler        = pickle.load(open(self.scaler_filename, 'rb'))
    ...
    def __str__(self):
        return "NBA Elastic Net Regression Model"
    ...
    def predict_game(self, game):
        ...
        # Generate predictions for home and away team scores
        home_score = round(float(self.home_eln_reg.predict(game_scaled)),1)
        away_score = round(float(self.away_eln_reg.predict(game_scaled)),1)
    ...
    def plot_rmse(self):
        ...
        for sch in schs:
            if sch.game_stats_filled and sch.team_stats_filled and sch.game_odds_filled and not sch.early_season_game:
                game_pred = NBAPredictions2022.objects.get(id=sch.id)
                home_rmse.append(game_pred.home_points_eln_cum_rmse)
                away_rmse.append(game_pred.away_points_eln_cum_rmse)
                home_vegas_rmse.append(game_pred.home_points_vegas_cum_rmse)
                away_vegas_rmse.append(game_pred.away_points_vegas_cum_rmse)
        ...
        plt.plot(idx, home_vegas_rmse, 'blue')
        plt.plot(idx, away_vegas_rmse, 'red')
        plt.title('NBA Elastic Net Regression Root Mean Squared Error')
        plt.xlabel('Number of Games Predicted')
        plt.ylabel("RMSE")
        plt.legend(labels=["Home Score Prediction", "Away Score Prediction", "Home Score Baseline", "Away Score Baseline"], bbox_to_anchor=(1.42, 0.5), loc='center right')
        plt.grid(True)
        save_name = PLOT_PATH + "nba_eln_reg_rmse.png"
        plt.savefig(save_name, bbox_inches='tight')
        plt.show(block=False)
    ...
    def plot_me(self):
        ...
        for sch in schs:
            if sch.game_stats_filled and sch.team_stats_filled and sch.game_odds_filled and not sch.early_season_game:
                game_pred = NBAPredictions2022.objects.get(id=sch.id)
                home_me.append(game_pred.home_points_eln_cum_me)
                away_me.append(game_pred.away_points_eln_cum_me)
                home_vegas_me.append(game_pred.home_points_vegas_cum_me)
                away_vegas_me.append(game_pred.away_points_vegas_cum_me)
        ...
        plt.plot(idx, home_vegas_me, 'blue')
        plt.plot(idx, away_vegas_me, 'red')
        plt.title('NBA Elastic Net Regression Mean Error')
        plt.xlabel('Number of Games Predicted')
        plt.ylabel("Mean Error")
        plt.legend(labels=["Home Score Prediction", "Away Score Prediction", "Home Score Baseline", "Away Score Baseline"], bbox_to_anchor=(1.42, 0.5), loc='center right')
        plt.grid(True)
        save_name = PLOT_PATH + "nba_eln_reg_me.png"
        plt.savefig(save_name, bbox_inches='tight')
        plt.show(block=False)